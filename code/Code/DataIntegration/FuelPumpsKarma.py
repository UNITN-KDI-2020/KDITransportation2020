...
if __name__ == "__main__":
    ...
    json_file_path = '../../dataset/Formal Modeling/data/fuel_pumps_assured.json'

    with open(json_file_path, 'r') as j:
        fuel_pumps = json.loads(j.read())
        #fuel_pumps = json.loads('../../dataset/Formal Modeling/data/fuel_pumps_assured.json')

        print(json.loads(fuel_pumps[0]))

        print("Found: {} records in the dataset.".format(len(fuel_pumps)))

        for i in range(len(fuel_pumps)):
            gas_station = fuel_pumps[i]
            if i%300 == 0:
                print("Processing {}/{} record.".format(i, len(fuel_pumps)))            
            record = json.loads(gas_station)
            ...
            Facility.loc[i] = [i, None, None, i, 0, i, None]
